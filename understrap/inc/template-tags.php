	...
	function understrap_posted_on() {
		...
		$posted_on   = apply_filters(
			'understrap_posted_on', sprintf(
				...
				esc_html_x( 'Опубліковано', 'post date', 'understrap' ),
		...
		$byline      = apply_filters(
			'understrap_posted_by', sprintf(
				...
				$posted_on ? esc_html_x( 'від', 'post author', 'understrap' ) : esc_html_x( 'Posted by', 'post author', 'understrap' ),
	...
	function understrap_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list && understrap_categorized_blog() ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Категорії %s', 'understrap' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'understrap' ) );
			if ( $tags_list ) {
				/* translators: %s: Tags of current post */
				printf( '<span class="tags-links">' . esc_html__( 'Мітки %s', 'understrap' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
